...
public class SerializeSG {
	...
	public static void main(String[] args) {
		...
		final String targetDir = args.length > 1 ? args[1] : "/elnelson/sg/";
		try {
			Configuration conf = new Configuration();
			final Configuration finalConf = new Configuration(conf);
			final FileSystem fs = FileSystem.get(conf);
			final TSerializer ser = new TSerializer(new TJSONProtocol.Factory());
			SGProcessor processor = new SGProcessor() {
				Map<LocalDate, Writer> dateMap = new HashMap<LocalDate, Writer>();
				LocalDate currentDate = LocalDate.now();

				Writer getWriter(String fileName) throws IOException {
					if(!dateMap.containsKey(currentDate)) {
						dateMap.put(currentDate,
								SequenceFile.createWriter(finalConf,
										SequenceFile.Writer.file(
												new Path(targetDir + "/sgfile-" + currentDate)),
										SequenceFile.Writer.keyClass(IntWritable.class),
										SequenceFile.Writer.valueClass(BytesWritable.class),
										SequenceFile.Writer.compression(CompressionType.NONE)));
					}
					return dateMap.get(currentDate);
				}

				@Override
				void processSG(weekPOI wkpoi, String fileName) throws IOException {
					try {
						getWriter(fileName).append(new IntWritable(1), new BytesWritable(ser.serialize(wkpoi)));;
					} catch (TException e) {
